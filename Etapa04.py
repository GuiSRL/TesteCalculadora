...
def logaritimo(numero, base):
    """Retorna o logaritma de um número na base específicada"""
    if numero <= 0 or base <= 0 or base == 1:
        raise ValueError ("Numero deve ser positivo e a base positiva diferente de 1.")
    return math.log(numero, base)

try:
    num1_str = input("Digite o primeiro numero: ")
    operador = input("Digite o operador: ")
    num2_str = input("Digite o segundo numero ( ou raiz/expoente/base para outras operacoes): ")

    # Passo 1: Verificar se os números inseridos são válidos.
    if not num1_str.replace('.', '', 1).isdigit() or not num2_str.replace('.', '', 1).isdigit():
        raise ValueError("Por favor, insista numeros validos.")
    else:
        num1 = float(num1_str)
        num2 = float(num2_str)
        resultado = None
    ...
    if operador in ['+', '-', '*', '/', '**', 'rad', 'sen', 'cos', 'tan', 'log']:
        ...
        if operador == '+':
            ...
            resultado = seno(num1) # Assume que o primeiro é o ângulo em graus
        elif operador == 'cos':
            resultado = cosseno(num1) # Assume que o primeiro é o ângulo em graus
        elif operador == 'tan':
            resultado = tangente(num1) # Assume que o primeiro é o ângulo em graus
        elif operador == 'log':
            resultado = logaritimo(num1, num2)

        # Passo 5: Exibir o resultado.
        print(f"O resultado é: {resultado}")
    else:
        # Passo 3: Exibir as mensagens de erros apropriadas.
